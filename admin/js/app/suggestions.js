        ...
        var call_back = function (result) {
            if (result.error == 0) {
                $.each(result.suggestions, function (index, obj) {
                    var suggestion = $('<tr><td data-id="' + obj.id + '">' + obj.title + '</td><td><button type="button" class="am-btn am-btn-success">查看</button></td></tr>')
                ...
                $('#suggetion_list').find('.am-btn-success').click(function () {
                    var $btn = $(this)
                    var suggestid = $($btn.parent().parent().children()[0]).data('id')
